...
{
    public partial class FloatingBarPage : Page
    {
        private bool _isLoaded = false;

        public FloatingBarPage()
        {
            InitializeComponent();
            Loaded += Page_Loaded;
        ...
        private void Page_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            // 嵌入工具栏组件页面
            if (ToolbarComponentsFrame.Content == null)
                ToolbarComponentsFrame.Navigate(new ToolbarPage());

            // 嵌入工具栏菜单页面
            if (ToolbarMenuFrame.Content == null)
                ToolbarMenuFrame.Navigate(new ToolbarMenuPage());
        }
        ...
        private void ToolbarComponentsFrame_Navigating(object sender, NavigatingCancelEventArgs e)
        {
            // Allow only ToolbarPage navigation
            if (e.Content is not ToolbarPage)
                e.Cancel = true;
        }

        private void ToolbarMenuFrame_Navigating(object sender, NavigatingCancelEventArgs e)
        {
            // Allow only ToolbarMenuPage navigation
            if (e.Content is not ToolbarMenuPage)
                e.Cancel = true;
        }
        ...
        private void LoadSettings()
        {
            ...
            // 加载彩色浮动栏设置
            if (CardIsColorfulViewboxFloatingBar != null)
                CardIsColorfulViewboxFloatingBar.IsOn = settings.Appearance.IsColorfulViewboxFloatingBar;
        }
        ...
        private void UpdateFloatingBarActualScaleText()
        {
            ...
            ViewboxFloatingBarActualScaleText.Text = $"{clampedVal:F2}x";
        }
        ...
        private void ViewboxFloatingBarScaleTransformValueSlider_ValueChanged(object sender, RoutedEventArgs e)
        {
            ...
            SettingsActionHub.OnFloatingBarScaleChanged(clampedVal);
        }
        ...
        #region Toolbar Behavior Toggles
        ...
        {
            if (!_isLoaded) return;
            SettingsManager.Settings.Appearance.IsColorfulViewboxFloatingBar = CardIsColorfulViewboxFloatingBar.IsOn;
            SettingsManager.SaveSettingsToFile();
        }
